...
class MicrosoftOutlookAttachments_Tools implements INode {
    ...
    constructor() {
        ...
        this.inputs = [
            ...
            {
                label: 'Attachment [Send Message]',
                name: 'attachmentSendMessage',
                type: 'json',
                description: 'Single file or array of files. Each item: { name, mimeType, base64 } (base64 without data: prefix)',
                show: {
                    outlookType: ['message'],
                    messageActions: ['sendMessage']
                },
    ...
    transformNodeInputsToToolArgs(nodeData: INodeData): Record<string, any> {
        ...
        if (nodeData.inputs?.attachmentSendMessage) defaultParams.attachmentSendMessage = nodeData.inputs.attachmentSendMessage
