...
// --- Add this helper schema above SendMessageSchema (e.g. near other schemas) ---
const AttachmentItem = z.object({
    name: z.string().optional().describe('File name, e.g., test.pdf'),
    mimeType: z.string().optional().describe('MIME type, e.g., application/pdf'),
    base64: z.string().optional().describe('Base64 content (without data: prefix)'),
    data: z.any().optional().describe('Binary/Buffer/Uint8Array'),
    url: z.string().optional().describe('URL to fetch the file from')
})

// --- Replace your existing SendMessageSchema with this ---
const SendMessageSchema = z.object({
    to: z.string().describe('Recipient email address(es), comma-separated'),
    subject: z.string().optional().describe('Subject of the message'),
    body: z.string().optional().describe('Body content of the message'),
    attachmentSendMessage: z.union([AttachmentItem, z.array(AttachmentItem)]).optional()
})
...
class SendMessageTool extends BaseOutlookTool {
    ...
    async _call(arg: any): Promise<string> {
        ...
        // ---- التقاط القيم من المسطّح أو المتعشّق ----
        const to = params.to ?? params.toSendMessage ?? params.sendMessage?.to

        const subject = params.subject ?? params.subjectSendMessage ?? params.sendMessage?.subject

        const body = params.body ?? params.bodySendMessage ?? params.sendMessage?.body

        // المرفقات: نقبل attachmentSendMessage أو attachment أو sendMessage.attachmentSendMessage
        const rawAtt = params.attachmentSendMessage ?? params.attachment ?? params.sendMessage?.attachmentSendMessage
        ...
        const toBase64 = async (v: any): Promise<string> => {
            if (!v) return ''
            if (typeof v === 'string') return stripDataUrl(v) // base64 أو Data URL
            ...
            if (v?.data) return toBase64(v.data) // { data: <binary> }

            if (v?.url) {
                const res = await fetch(v.url)
                if (!res.ok) throw new Error(`Failed to fetch attachment from URL: ${res.status} ${res.statusText}`)
        ...
        const items = Array.isArray(rawAtt) ? rawAtt : rawAtt ? [rawAtt] : []

        // Build Microsoft Graph attachments
        const attachments = []
        for (const f of items) {
            const name = (f?.name as string) || 'attachment'
            const contentType = (f?.mimeType as string) || guessMime(name)
            const contentBytes = await toBase64(f?.base64 ?? f?.data ?? f)
        ...
        try {
            const messageData = {
                message: {
                    ...
                    toRecipients: this.parseEmailAddresses(String(to || '')),
